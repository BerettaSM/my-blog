...
def admin_only(f):
    ...
    def decorated_function(*args, **kwargs):
        is_admin = getattr(current_user, 'id', -1) == 1
        if is_admin:
            return f(*args, **kwargs)
        return abort(HTTPStatus.FORBIDDEN, "You don't have permission to view that resource.")
...
def register():
    ...
    if form.validate_on_submit():
        if email_already_registered(email=form.email.data):
